classdef measurement < handle
    ...
    methods
        ...
        function init(obj)
            ...
            obj.device.write('MEAS{IRIM}');
            % Sets oscillation
            obj.device.write(['POWE ', num2str(par.osc_level) ' V']);
            % Sets the sweep range start value. P444
            obj.device.write(['STAR ', num2str(par.leftFq), 'HZ']);
            % Sets the sweep range stop value. P446
            obj.device.write(['STOP ', num2str(par.rightFq), 'HZ']);
            ...
            obj.device.write(['POIN ', num2str(par.NOP)]);
            % Enable average measurement points
            obj.device.write('PAVER ON');
            % Points averaging factor
            obj.device.write(['PAVERFACT ', num2str(par.p_aver)]);
        end

        function run(obj)
            freqVector = linspace(par.leftFq, par.rightFq, par.NOP);
            ...
            while ishandle(keyHandle)
                [mag, phs] = obj.device.oneSweep(ia, par);
                ...
                % save freq array
                dlmwrite([par.dir, par.log, '_freq.csv'], [timeNow, freqVector], 'delimiter', ',', '-append'); % append mode
                % save mag parts
                dlmwrite([par.dir, par.log, '_real.csv'], [timeNow, transpose(mag)], 'delimiter', ',', '-append');
                % save phs parts
                dlmwrite([par.dir, par.log, '_phs.csv'], [timeNow, transpose(phs)], 'delimiter', ',', '-append');
                % analyze data
